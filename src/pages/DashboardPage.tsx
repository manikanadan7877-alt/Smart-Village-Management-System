...
import {
  Users, Droplets, Sprout, HeartPulse, Cloud,
  AlertTriangle, Plus, Send, ClipboardList, Bot,
  Activity, MapPin, Radio, CloudRain, Wind,
  Layers, Eye, Sparkles,
} from 'lucide-react';
...
export function DashboardPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const [c, tk, b] = await Promise.all([
          fetchComplaints(),
          fetchWaterTanks(),
          fetchGarbageBins(),
        ]);
  ...
  const avgWaterPct = waterTanks.length > 0
    ? Math.round(waterTanks.reduce((sum, tk) => sum + (tk.capacity_liters > 0 ? (tk.current_level_liters / tk.capacity_liters) * 100 : 0), 0) / waterTanks.length)
    : 0;
  const lowWaterTanks = waterTanks.filter((tk) => {
  ...
  lowWaterTanks.forEach((tk) => {
    const pct = (tk.current_level_liters / tk.capacity_liters) * 100;
    alerts.push({
      type: 'water', title: `${t('dash.waterStatus')}: ${tk.name}`, subtitle: `${pct.toFixed(0)}%`,
  ...
  fullBins.forEach((b) => {
    ...
    alerts.push({
      type: 'waste', title: `${t('nav.waste')}: ${b.name}`, subtitle: `${pct.toFixed(0)}%`,
    ...
    <div className="cmd-bg cmd-grid-pattern -m-4 min-h-[calc(100vh-4rem)] rounded-tl-2xl p-4 lg:p-6 space-y-5">
      ...
      <div className="cmd-fade-up" style={{ animationDelay: '0s' }}>
        ...
        <p className="text-sm text-slate-400 mt-1">
          {t('dash.overview')}
          {villageName && <span className="text-green-400 font-medium"> {villageName}</span>}
        </p>
      </div>

      {/* Top status cards */}
      <div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-5 cmd-fade-up" style={{ animationDelay: '0.05s' }}>
        ...
        <StatusCard
          ...
          value={avgWaterPct >= 60 ? t('dash.good') : avgWaterPct >= 30 ? 'Moderate' : 'Critical'}
          icon={Droplets}
          color={avgWaterPct >= 60 ? '#22c55e' : avgWaterPct >= 30 ? '#f59e0b' : '#ef4444'}
          subtitle={`${waterTanks.length} ${t('dash.tanks')} • avg ${avgWaterPct}%`}
