...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const hasTanglish = /\b(epdi|irukku|irukka|sollu|enna|eththani|engal|oor|veli|pani|thanneer|vilai|makal|village status|water status|agriculture|weather|alert|complaint|electricity|crop)\b/i.test(query) && !hasTamil;
    const isTamil = hasTamil;
    const isTanglish = hasTanglish;

    // Build response
    const { reply, module } = buildReply(query, data, { lowWaterTanks, fullBins, highPriorityCount }, isTamil, isTanglish);
...
function buildReply(
  ...
  // Energy / electricity
  if (/\b(energy|electric|power|solar|light|street light|viduthi|மின்|bijli)\b/i.test(query)) {
    if (isTamil) return { reply: `மின் நுகர்வு: 2,450 kWh. சோலார் உற்பத்தி: 1,820 kWh. 48/50 தெரு விளக்குகள் செயல்படுகின்றன. 2 விளக்குகள் செயல்படவில்லை (Market Road).`, module: '/energy' };
    if (isTanglish) return { reply: `Electricity usage: 2,450 kWh. Solar: 1,820 kWh generated. 48/50 street lights working. 2 lights not working on Market Road.`, module: '/energy' };
    return { reply: `Energy status: Daily consumption 2,450 kWh. Solar generation: 1,820 kWh. 48 out of 50 street lights are working. 2 lights are not working on Market Road — repair recommended.`, module: '/energy' };
  }
  ...
  if (/\b(prediction|forecast|ai|predict|risk)\b/i.test(query)) {
    if (isTamil) return { reply: `AI கணிப்பு: நீர் பற்றாக்குறை - குறைந்த ஆபத்து. பயிர் விளைச்சல் - அதிக விளைச்சல். மின் தேவை - சாதாரண. நோய் பரவல் - குறைந்த ஆபத்து.`, module: '/analytics' };
    if (isTanglish) return { reply: `AI Prediction: Water shortage - Low Risk. Crop yield - High Yield. Power demand - Normal. Disease outbreak - Low Risk.`, module: '/analytics' };
    return { reply: `AI Predictions: Water shortage — Low Risk. Crop yield — High Yield expected. Power demand — Normal. Disease outbreak — Low Risk. Flood risk — Low. Infrastructure risk — Moderate (Market Road).`, module: '/analytics' };
  }

  // Default
  if (isTamil) return { reply: `உங்கள் கேள்வத்தை புரிந்துகொள்ள முடியவில்லை. நீர், வேளாண்மை, மின்சாரம், குப்பை, புகார், வானிலை, மருத்துவம் பற்றி கேளுங்கள்.`, module: null };
  if (isTanglish) return { reply: `Unga kelvi puriyala. Water, agriculture, electricity, waste, complaint, weather, health pathri kelunga.`, module: null };
  return { reply: `I didn't understand your question. You can ask me about: water status, agriculture, energy, waste management, complaints, weather, healthcare, village overview, or AI predictions.`, module: null };
}
